...
export const scanQRAttendance = async (req, res) => {
  try {
    ...
    const isEnrolled = attendance.records.some(
  ...
  try {
    ...
    const history = attendanceSessions.map((session) => {
      ...
      return {
        ...
        wasLate:
          record.timeScanned && record.timeScanned > session.qrCode.expiresAt,
      };
    });
        ...
        totalSessions > 0
          ? Math.round(
              (history.reduce((sum, session) => {
                return (
                  sum + session.attendanceStats.present / session.totalStudents
                );
              }, 0) /
                totalSessions) *
                100
